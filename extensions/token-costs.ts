...
// ─── Suffix stripping constants ──────────────────────────────────────
/** Common suffixes that don't affect model-family identity. */
const COMMON_SUFFIXES =
	/[-_\s](?:instruct|chat|turbo|h|gq|hf|i1)$|\b(?:free|latest)\b/i;

/** Extract base model family by stripping parameter-count + variant suffixes.
 *  "qwen3-6-35b-a3b" → "qwen3-6", "llama-3-3-70b" → "llama-3-3",
 *  "gemma-4-26b-a3b-qat" → "gemma-4". */
function extractBaseModel(normalized: string): string {
	// Match -<digits><B/b/K> optionally followed by variant tags (-a3b, -qat, etc.)
	const result = normalized.replace(/-\d+[bmBMk]+(?:-[a-z0-9]+)*$/, "");
...
/** Strip parameter-count suffixes from a normalized model ID.
 *
 *  Only strips param counts (e.g., -70b, -35b) that appear at the END of the
 *  string. This is used as ONE strategy among many in findLivePriceByStripping;
 *  other strategies handle params that appear mid-string or require deeper
 *  stripping.
 *
 *  Examples:
 *    llama-3-3-70b         → llama-3-3
 *    gemma4-26b-a3b-qat    → gemma4-26b-a3b (a3b/qat NOT stripped here — see findLivePriceByStripping)
 *    qwen3-6-instruct      → qwen3-6-instruct  (instruct is semantic, not a param count)
 *    phi4                   → phi4              (no trailing param count)
 */
function stripParameterSuffix(normalized: string): string {
	return normalized.replace(/-\d+[bmBMk]+$/, "").trim();
}

/** Extract parameter-count segments from a normalized ID.
 *
 *  Returns an array of param-count matches found anywhere in the string,
 *  e.g. "qwen3-6-35b-a3b" → [{ segment: "-35b", start: 9, end: 14 }].
 *  This is used to try matching WITH param counts preserved before falling
 *  back to stripped family-level keys.
 */
function extractParamCounts(
	normalized: string,
): { segment: string; start: number; end: number }[] {
	const results: { segment: string; start: number; end: number }[] = [];
	const regex = /(-\d+[bmBMk]+(?:-[a-z0-9]+)*)/g;
	let match;
	while ((match = regex.exec(normalized)) !== null) {
		// Only treat it as a param count if the segment starts with -<digits><B/b/K>
		if (/^-\d+[bmBMk]+$/.test(match[1])) {
			results.push({
				segment: match[1],
				start: match.index,
...
function resolveModel(modelId: string): {
	...
	// 4b. Fallback local DB match → also check for online equivalent via aliases
	for (const [key, price] of Object.entries(LOCAL_MODEL_DEFAULTS)) {
		const keyLower = key.toLowerCase();
		if (
			normalized === keyLower ||
...
function lookupPrice(modelId: string): { input: number; output: number } {
	return resolveModel(modelId).price;
}
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		if (ctx.model?.id) {
			...
			if (resolved.price.input > 0 || resolved.price.output > 0) {
				ctx.ui.notify(
					`Token tracking: ${name} — $${resolved.price.input.toFixed(5)} input / $${resolved.price.output.toFixed(5)} output per M tokens (${resolved.source})`,
					"info",
				);
			} else {
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			return {
				content: [
					{
						type: "text",
						text: `Recorded ${fmtTokens(params.inputTokens)} input + ${fmtTokens(params.outputTokens)} output tokens for ${modelName(modelId)}. Est. cost: ${fmtCost(entry.costTotal)}`,
					},
				],
			};
		},
